...
-- WRAPPER for custum AI
function runkey(i)
	return objects["player"][i].action == 3 or objects["player"][i].action == 4
end

function rightkey(i)
	return objects["player"][i].action == 1 or objects["player"][i].action == 3
end

function leftkey(i)
	return objects["player"][i].action == 2 or objects["player"][i].action == 4
end

function downkey(i)
	return objects["player"][i].action == 5
end

function upkey(i)
	return objects["player"][i].action == 6
end
-- END

function handle_screenshot(screenshot)
	print("screen_shots/" .. ticks .. "-test.png")
	love.filesystem.write("screenshots/" .. ticks .. "-test.png", screenshot:encode("png"))
end

marioAI = class:new()

function marioAI:init(x, y, i, animation, size, t)
	-- Custom variables
	self.action = 0
	self.consec_jumps = 0
	self.last_mario_time = 400
...
function marioAI:set_action(action)
	-- set the correct key bindings
	self.action = action
end

function marioAI:execute_action()
--	love.graphics.captureScreenshot(handle_screenshot)

	if not (self.action == 6) then
		self.consec_jumps = 0
		self:stopjump()
	end

	-- walk or run right
	if self.action == 1 or self.action == 3 then
		self:rightkey()
	-- walk or run left
	elseif self.action == 2 or self.action == 4 then
		self:leftkey()
	-- jump
...
function marioAI:underwatermovement(dt)
	if self.jumping or self.falling then
		--Swim animation
...
function marioAI:setquad(anim, s)
...
function marioAI:floorcollide(a, b)
	self.rainboomallowed = true
	...
	--Make marioAI snap to runspeed if at walkspeed.
...
function marioAI:stompenemy(a, b)
	local bounce = false
	if a == "koopa" then
		if b.small then
			playsound(shotsound)
...
function marioAI:rightcollide(a, b)
	if self:globalcollide(a, b) then
		return false
	end
	...
	elseif a == "tile" then
		...
		--Check if marioAI should run across a gap.
...
function marioAI:leftcollide(a, b)
	if self:globalcollide(a, b) then
		return false
	end
...
function marioAI:ceilcollide(a, b)
	if self:globalcollide(a, b) then
		return false
	end
	...
	elseif a == "tile" then
		...
		if tilequads[map[x][y][1]].invisible then
			...
			--Check if it should bounce the block next to it, or push marioAI instead (Hello, devin hitch!)
...
function marioAI:globalcollide(a, b)
...
function marioAI:passivecollide(a, b)
	if self:globalcollide(a, b) then
		return false
	end
...
function marioAI:starcollide(a, b)
	--enemies that die
...
function marioAI:faithplate(dir)
	self.animationstate = "jumping"
	self.falling = true
	self:setquad()
end

function marioAI:startfall()
	if self.falling == false then
		self.falling = true
		self.animationstate = "falling"
		self:setquad()
...
function marioAI:removeportals(i)
...
function marioAI:pickupbox(box)
	self.pickup = box
end

function marioAI:dropbox()
	self.pickup:dropped()
...
function marioAI:cubeemancipate()
	self.pickup = nil
end

function marioAI:duck(ducking) --goose
...
function marioAI:pipe(x, y, dir, i)
	self.active = false
...
function marioAI:vineanimation()
	self.animation = "vine"
	self.invincible = false
	self.drawable = true
	self.controlsenabled = false
...
function marioAI:fireballcallback()
	self.fireballcount = self.fireballcount - 1
end
